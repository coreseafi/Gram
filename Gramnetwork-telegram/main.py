...
def get_available_scripts(language):
    scripts = {
        'vi': [
            {"name": "1. Tự động Boost (Energy & Power)", "value": "boost"},
            {"name": "2. Tự động làm nhiệm vụ", "value": "social"},
            {"name": "3. Tự động đào $GRM", "value": "mining"},
          
            {"name": "X. Thoát", "value": "exit"},
        ],
        'en': [
            {"name": "1. Automatic Boost (Energy & Power)", "value": "boost"},
            {"name": "2. Automatic complete tasks", "value": "social"},
            {"name": "3. Automatic mining $GRM", "value": "mining"},
...
if __name__ == "__main__":
    if os.getenv("RAILWAY_ENVIRONMENT") or os.getenv("AUTO_RUN", "").lower() in ("1", "true", "yes"):
        from run_auto import main as auto_main
        auto_main()
    else:
        main()
